...
class CONTROLLER:
    ...
    def displayDataToTreeView_ExcelFile(self):
        self.view.treeview.delete(*self.view.treeview.get_children())
        if ('.xlsx' in self.model.file_name):
            ...
            messagebox.showerror(title='Invalid file', message='Please select .xlsx if Window, .ods if Linux(Ubuntu) file! Thank you !')

    def GenerateData(self):
        if(self.check):
            if ('.csv' in self.model.file_name):
                data = pd.read_csv(self.model.file_name)
            elif ('.xlsx' in self.model.file_name):
